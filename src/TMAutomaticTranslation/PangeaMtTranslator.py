...
class PangeaMtTranslator(Translator):

  def __init__(self, **kwargs):
    config = ENGINE_CONFIG.get_engine('pangeamt')
    self.URL = config['url']
  ...
  def _translate(self, text_list, to_lang, from_lang):
    params = {"src": from_lang,
              ...
              "apikey": self.API_KEY}
    try:
      result = requests.post(self.URL + "/v1/translate",
                            headers={"Content-Type": "application/json"},
                            data=json.dumps(params))
    except:
      logging.info("PangeaMT: failed to translate: {}, reason: {}".format(text_list, sys.exc_info()[1]))
      return None
